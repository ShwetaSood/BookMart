%Improves accuracy for eg. 150 user Suchita likes fiction, travel, fantasy, drama
%the most. Only those books in these categories are placed on higher rating 
%compared to others.Whereas in earlier method, it gave equal imp to all features coz of which
%some books like tintin were recommended to suchita that was political,
%education and detective type apart from being in fantasy and fiction. However, she had never read any book of
%political side and had read less books from education and detective. 
%By providing weights based on what categories are read more by user, we
%were able to place books solely based on travel, fantasy,
%drama,fiction on higher priority compared to other categories liked less or not liked at all.
...
for i=2:size(usertxt,1)
    ...
    for j=1:16
        cat(j)=sum(union_arr(:,j));
    end
    for j=2:17
        user_matrix(i,j)={cat(j-1)/max(cat)};
    end
end
